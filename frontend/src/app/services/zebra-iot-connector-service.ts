...
export class ZebraIoTConnectorService implements OnInit {
  ...
  public newTagAccessOpEvent: EventEmitter<ReadTagEventModel> = new EventEmitter<ReadTagEventModel>();
  ...
  setGPOState(port : Number, state : Boolean) {
    
      let payload = {
        "port": port,
        "state" : state
      }
      this._mqttService.unsafePublish(this.managementInterfaceCmd, new SendCommandModel("set_gpo", payload).toJson(), { qos: 0, retain: false });
  }
/*
